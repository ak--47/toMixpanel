...
import { touch, mkdir } from 'ak-tools';
...
async function amplitudeETL(config, directoryName) {
	...
	let importedProfiles = await mpImport(creds, usersDir, { recordType: `user`, logs: true, region, recordsPerBatch: config?.destination?.options?.recordsPerBatch || 2000 });
    ...
    ${smartCommas(importedProfiles.results?.totalRecordCount)} users merged
    ${smartCommas(importedMergeTables.results?.totalRecordCount)} profiles updated`);

	// write logs
	const logsDir = path.resolve(`./savedData/${directoryName}/logs`);
	await mkdir(logsDir)
	await touch(path.resolve(`${logsDir}/eventLogs.json`), importedEvents.responses, true, false, false)
	await touch(path.resolve(`${logsDir}/profileLogs.json`), importedProfiles.responses, true, false, false)
	await touch(path.resolve(`${logsDir}/mergeLogs.json`), importedMergeTables.responses, true, false, false) 
